...
function WebRtcPeer(mode, options, callback) {
  ...
  this.checkConnectionFailed = function (callback) {
    if (!pc) return;

    function setConnectionStateChangeAccordingWebBrowser(functionToExecute, pc) {
      if (typeof AdapterJS !== 'undefined' && AdapterJS.webrtcDetectedBrowser ===
        'IE' && AdapterJS.webrtcDetectedVersion >= 9) {
        pc.onconnectionstatechange = functionToExecute;
      } else {
        pc.addEventListener('connectionstatechange', functionToExecute);
      }
    }

    // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/iceconnectionstatechange_event
    var connectionFailedFunction = function (event) {
      if (['disconnected', 'failed'].includes(event.target.connectionState)) {
        callback();
      }
    };

    setConnectionStateChangeAccordingWebBrowser(connectionFailedFunction, pc);
  }
